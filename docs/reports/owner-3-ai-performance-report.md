...
2026-07-03 fresh RAG retrieval benchmark 기준으로 전역 vector 유지, 경로별 override 기본값 유지다. `REQUIREMENT_PARSE`, `BUILD_RECOMMEND`, `AS_ANALYZE`, `BUILD_EXPLAIN`, `PUBLIC_SEARCH` 모두 topK 100%를 통과했고, 후속 개선은 top1 안정성에 집중한다.

2026-07-03 Build Chat 속도 개선 기준으로 장바구니 교체/삭제/수량 변경처럼 고확신 draft action은 LLM 호출 전 fast path에서 처리한다. 전체 66개 live case 평균은 2,271ms이고, 전 카테고리 교체 방향성 24개 case는 평균 16ms, p95 32ms로 측정됐다. 풀 견적 생성과 설명처럼 판단 근거 합성이 필요한 요청은 계속 `BUILD_CHAT_54_MINI_FAST` LLM/RAG 경로를 사용한다.

2026-07-03 이후 웹 체감 속도 판단은 API 직접 호출 benchmark와 분리한다. source of truth는 `docs/reports/web-ai-latency-YYYYMMDD.md`와 `.json`이며, 실제 브라우저에서 챗봇 입력, 자동 route/action, navigation, draft 갱신까지 포함해 측정한다.

## 2026-07-03 웹 체감 AI Latency 검증

실행 기준:

```powershell
docker compose up --build -d
npm --prefix apps/web run test:e2e:ai-latency
```

측정 대상은 Docker live web `http://127.0.0.1:5173`와 API `http://127.0.0.1:8080`이다. 공통 계정은 `user@example.com / passw0rd!`를 사용한다.

합격 기준:

| group | 목적 | p95 기준 | 단일 케이스 기준 |
| --- | --- | ---: | ---: |
| FAST_LOCAL_ROUTE | 프론트 즉시 이동 shortcut | 1.0초 이하 | 5.0초 이하 |
| FAST_SERVER_ROUTE | 서버 상품/필터 route resolver | 1.5초 이하 | 5.0초 이하 |
| DRAFT_ACTION | 장바구니 자동 변경 action | 2.0초 이하 | 5.0초 이하 |
| DETERMINISTIC_RECOMMEND | 고확신 추천/부품 상담 | 3.0초 이하 | 5.0초 이하 |
| LLM_FULL_COMPLEX | 복합 의도 LLM/RAG/Tool 경로 | 5.0초 이하 | 5.0초 이하 |

보고서에는 `submitStart`, `assistantRendered`, `actionStarted`, `actionSettled`, `navigationSettled`, `draftSettled`, `totalMs`를 남긴다. Markdown에는 평균/p95/max를 밀리초와 초 단위로 함께 병기한다.
...
- Build Chat live: 66개 case. 추천 3개 생성, 5090 하드 제약, 부품 추천, 전 카테고리 견적 draft action을 포함한다.

2026-07-03 fresh retrieval 결과:

| purpose | cases | top1 hit | topK hit | avg latency |
| --- | ---: | ---: | ---: | ---: |
| REQUIREMENT_PARSE | 90 | 78.9% | 100.0% | 297ms |
| BUILD_RECOMMEND | 20 | 75.0% | 100.0% | 298ms |
| AS_ANALYZE | 50 | 78.0% | 100.0% | 303ms |
| BUILD_EXPLAIN | 10 | 100.0% | 100.0% | 297ms |
| PUBLIC_SEARCH | 20 | 90.0% | 100.0% | 305ms |
...
Build Chat draft action 속도 회귀는 별도 case group으로 빠르게 확인한다.

```powershell
python tools/benchmark_build_chat_profiles.py --profiles BUILD_CHAT_54_MINI_FAST --variant-label fastpath-v1 --case-group DIRECTION_MATRIX --strict
```

2026-07-03 fast path 측정 결과:

| 범위 | cases | successRate | avgLatencyMs | p95LatencyMs | maxLatencyMs | fastPathUsedRate |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 전체 Build Chat live | 66 | 100.0% | 2,271 | 4,722 | 16,276 | 43.9% |
| 전 카테고리 교체 방향성 | 24 | 100.0% | 16 | 32 | 35 | 100.0% |

단일 16초 outlier는 `white-build` 풀 견적 생성 케이스였고, fast path 대상이 아니다.
...
Build Chat 반복 요청 속도 개선은 Redis cache로 보조한다. cache는 성공 응답만 TTL 600초로 저장하고, key는 사용자 내부 id, profile, 정규화 message, selectedCategory, currentQuoteDraft fingerprint, parts/benchmark/FPS/RAG/alias version을 포함한다. cache hit/miss는 사용자 응답 body에 노출하지 않으며 Redis 장애 시 기존 LLM 흐름으로 우회한다. cache hit 응답은 이전 실행의 `agentSessionId`, `evidenceIds`, `toolInvocationIds`를 재사용하지 않는다.
...
- 2026-07-01 LLM smoke만으로는 vector-on 품질 이득이 작아 보였지만, 2026-07-02 retrieval benchmark에서는 vector-off가 주요 경로에서 결과를 거의 반환하지 못해 품질 기준으로 탈락했다. 운영 기본값은 `RAG_VECTOR_ENABLED=true`를 유지한다.

권장 후속 조치:

1. 사용자 기본 profile을 `AS_CHAT_54_MINI_FAST`, `BUILD_CHAT_54_MINI_FAST`로 전환한다.
2. `RAG_VECTOR_ENABLED=true`를 유지하고, `REQUIREMENT_PARSE`/`BUILD_RECOMMEND`의 top1 안정성을 계속 개선한다.
