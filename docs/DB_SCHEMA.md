...
`agent_idempotency_records`는 내부 멱등성 기록 전용이라 `public_id`를 두지 않는다.
...
| AS ticket analysis status | `NOT_STARTED`, `QUEUED`, `ANALYZING`, `RULE_READY`, `LLM_READY`, `FAILED` |
| AS ticket review status | `NOT_REQUIRED`, `REQUIRED`, `IN_REVIEW`, `APPROVED`, `REJECTED` |
| AS chat session status | `ACTIVE`, `ARCHIVED` |
| AS chat message role | `USER`, `ASSISTANT` |
| Recommendation event type | `IMPRESSION`, `CLICK`, `DETAIL_VIEW`, `SAVE`, `CHANGE_ADOPTED`, `ADD_BUILD_TO_DRAFT`, `ADD_PART_TO_DRAFT`, `ORDER_INTENT`, `REJECT`, `CHANGE_REVERTED`, `AS_CONFIRMED_NEGATIVE`, `ADMIN_PROMOTE`, `ADMIN_DEMOTE` |
| Recommendation model status | `CANDIDATE`, `SHADOW`, `ACTIVE`, `RETIRED`, `FAILED` |
| Recommendation training dataset status | `DRAFT`, `LOCKED`, `TRAINED`, `ARCHIVED` |
| Recommendation training job status | `QUEUED`, `RUNNING`, `SUCCEEDED`, `FAILED`, `CANCELLED`, `SKIPPED_LOW_DATASET` |
...
| Pipeline job run status | `SUCCEEDED`, `FAILED`, `SKIPPED_FROZEN`, `SKIPPED_LOCKED` |
| Manufacturer source type | `NEWS`, `PRODUCT_RELEASE`, `SUPPORT_NEWS`, `RSS`, `SITEMAP` |
| Manufacturer source status | `ACTIVE`, `PAUSED`, `ERROR` |
| Manufacturer post classification status | `PENDING`, `PRODUCT_CANDIDATE`, `IGNORED`, `FAILED` |
| Manufacturer post classification source | `RULE`, `AI`, `ADMIN` |
...
- `analysis_status` 진행 방향: `NOT_STARTED` → `QUEUED` → `ANALYZING` → `RULE_READY` → `LLM_READY`. 어느 단계에서든 실패하면 `FAILED`다.
- `review_status` 진행 방향: `NOT_REQUIRED` 또는 `REQUIRED` → `IN_REVIEW` → `APPROVED` | `REJECTED`.
- `support_decision`(`SELF_SOLVABLE`, `REMOTE_POSSIBLE`, `VISIT_REQUIRED`, `NEEDS_MORE_INFO`)과 `risk_level`(`LOW`, `MEDIUM`, `HIGH`)은 분석/검토 결과로 채워지는 판정 값이며 상태 기계는 아니다.
- V1 구현 기준: PC Agent 데모 업로드 파이프라인은 룰 진단이 끝난 티켓을 `analysis_status='RULE_READY'`, `review_status='REQUIRED'`, `support_decision='NEEDS_MORE_INFO'`, `risk_level='MEDIUM'`, `auto_response_allowed=false`로 생성한다. 관리자 티켓 API는 `review_status`, `support_decision`, `auto_response_allowed`를 갱신할 수 있다.
- 이 상태들은 아직 service 전이표(409 강제)가 없고 DB CHECK 제약으로만 값 집합을 보장한다. 전이표 강제가 필요해지면 이 절과 `API_CONTRACT.md`의 409 조건을 함께 갱신한다.
...
manufacturer_sources
  └─ manufacturer_posts ── part_catalog_candidates

users
  └─ agent_devices
       ├─ agent_consents
       ├─ agent_heartbeats
       ├─ agent_upload_jobs ── agent_log_bundles ── agent_log_uploads
       ├─ agent_idempotency_records
       └─ agent_delete_requests

as_tickets
  ├─ agent_log_summaries ── as_ticket_labels
  ├─ remote_support_sessions
  └─ visit_support_reservations

recommendation_events ── parts / builds / as_tickets
  └─ recommendation_training_dataset_items
       └─ recommendation_training_datasets
            └─ recommendation_training_jobs ── recommendation_model_versions ── recommendation_shadow_scores
```

위 다이어그램은 도메인별 대표 관계만 표현한다. `build_graph_layouts`, `build_chat_semantic_cache`, `pipeline_job_runs`, `agent_activation_tokens`, `agent_update_policies`/`agent_update_rollouts` 같은 독립/운영 테이블은 생략한다.
...
목적: 공식 제조사 신제품/뉴스/릴리즈 페이지 감시 대상을 저장한다. 이 테이블은 사용자 요청 중 조회되지 않고 관리자 scan 또는 비활성 기본 스케줄러에서만 사용한다.
...
| `id` | `BIGINT` | no | - | 내부 PK |
| `public_id` | `UUID` | no | - | 외부 ID |
| `manufacturer` | `VARCHAR(100)` | no | - | 제조사명 |
| `category_scope` | `VARCHAR(50)` | no | - | `ALL` 또는 감시 category |
...
| `poll_interval_minutes` | `INTEGER` | no | - | scan 주기(분). 기본 1440, 허용 범위 30~10080 (DB CHECK). scanAll이 주기가 지난 소스만 스캔한다 (V92부터 실제 적용) |
...
- unique: active 상태에서는 `source_url` 중복을 허용하지 않는다.
- index: `manufacturer_sources.enabled`
- index: `manufacturer_sources.manufacturer`
- index: `manufacturer_sources.category_scope`
- index: `manufacturer_sources.deleted_at`

### manufacturer_posts

목적: `manufacturer_sources` scan 결과 새로 발견한 공식 게시글을 저장한다. 게시글은 신제품 후보인지 분류되며, 후보로 판단되면 네이버 검색을 통해 `part_catalog_candidates`와 연결된다.
...
| `id` | `BIGINT` | no | - | 내부 PK |
| `public_id` | `UUID` | no | - | 외부 ID |
| `manufacturer_source_id` | `BIGINT` | no | `manufacturer_sources.id` | 감지 source |
| `created_catalog_candidate_id` | `BIGINT` | yes | `part_catalog_candidates.id` | 생성된 후보 |
| `external_url` | `TEXT` | no | - | 공식 게시글 URL |
| `title` | `VARCHAR(500)` | no | - | 공식 게시글 제목 |
| `published_at` | `TIMESTAMPTZ` | yes | - | 게시 시각 |
| `content_hash` | `VARCHAR(128)` | no | - | 게시글 식별 hash |
...
| `classification_source` | `TEXT` | no | - | 분류 출처 `RULE`, `AI`, `ADMIN`. RULE만 재스캔이 분류를 갱신하며, AI/ADMIN 확정 상태는 재스캔이 덮어쓰지 않는다 (V90) |
...
- unique: active 상태에서는 `(manufacturer_source_id, external_url)` 중복을 허용하지 않는다.
- index: `manufacturer_posts.manufacturer_source_id`
- index: `manufacturer_posts.classification_status`
- index: `manufacturer_posts.detected_category`
- index: `manufacturer_posts.created_catalog_candidate_id`
- index: `manufacturer_posts.created_at`
- index: `manufacturer_posts.deleted_at`
...
| `intent` | `VARCHAR(80)` | no | - | `BUILD_RECOMMEND`, `PART_RECOMMEND` 등 router intent |
...
| `data_version_hash` | `VARCHAR(128)` | no | - | parts/benchmark/FPS/RAG/alias version hash |
| `expires_at` | `TIMESTAMPTZ` | no | - | 만료 시각 |
| `created_at` | `TIMESTAMPTZ` | no | - | 생성 시각 |
...
- unique: `build_chat_semantic_cache.public_id`
...
| `id` | `BIGINT` | no | - | 내부 PK |
| `public_id` | `UUID` | no | - | 외부 ID |
| `user_id` | `BIGINT` | no | `users.id` | 토큰 발급 대상 사용자 |
| `token_hash` | `VARCHAR(255)` | no | - | activation token hash |
| `expires_at` | `TIMESTAMPTZ` | no | - | 만료 시각 |
...
- unique: `agent_activation_tokens.public_id`
- unique: `agent_activation_tokens.token_hash`
- index: `agent_activation_tokens.user_id`
- index: `agent_activation_tokens.expires_at`
...
| `id` | `BIGINT` | no | - | 내부 PK |
| `public_id` | `UUID` | no | - | 외부 ID |
| `user_id` | `BIGINT` | no | `users.id` | 디바이스 소유 사용자 |
| `activation_token_id` | `BIGINT` | yes | `agent_activation_tokens.id` | 등록에 사용한 activation token |
| `device_fingerprint_hash` | `VARCHAR(255)` | no | - | 디바이스 fingerprint hash |
| `hostname_hash` | `VARCHAR(255)` | yes | - | hostname hash |
| `agent_token_hash` | `VARCHAR(255)` | no | - | agent Bearer token hash |
| `registration_idempotency_key` | `VARCHAR(160)` | no | - | 등록 멱등성 키 |
| `status` | `VARCHAR(30)` | no | - | `PENDING_REGISTERED`, `ACTIVE`, `UPDATE_REQUIRED`, `BLOCKED`, `REVOKED`, `UNINSTALLED` |
| `os_version` | `VARCHAR(120)` | no | - | OS 버전 |
| `agent_version` | `VARCHAR(50)` | no | - | Agent 버전 |
| `policy_version` | `VARCHAR(80)` | yes | - | 적용된 정책 버전 |
| `last_seen_at` | `TIMESTAMPTZ` | yes | - | 마지막 heartbeat 시각 |
| `blocked_reason` | `TEXT` | yes | - | 차단 사유 |
| `created_at` | `TIMESTAMPTZ` | no | - | 생성 시각 |
| `updated_at` | `TIMESTAMPTZ` | yes | - | 수정 시각 |

Index:

- unique: `agent_devices.public_id`
- unique: `agent_devices.agent_token_hash`
- unique: `(activation_token_id, registration_idempotency_key)`
- unique partial: `agent_devices.user_id` where `status IN ('PENDING_REGISTERED', 'ACTIVE', 'UPDATE_REQUIRED', 'BLOCKED')` — 사용자당 active 디바이스 1대
- index: `agent_devices.user_id`
...
| `id` | `BIGINT` | no | - | 내부 PK |
| `public_id` | `UUID` | no | - | 외부 ID |
| `device_id` | `BIGINT` | no | `agent_devices.id` | heartbeat 디바이스 |
| `agent_version` | `VARCHAR(50)` | no | - | 보고된 Agent 버전 |
| `service_status` | `VARCHAR(50)` | no | - | 백그라운드 서비스 상태 |
| `tray_status` | `VARCHAR(50)` | yes | - | 트레이 앱 상태 |
| `policy_version` | `VARCHAR(80)` | yes | - | 적용 정책 버전 |
| `idempotency_key` | `VARCHAR(160)` | no | - | 멱등성 키 |
| `received_at` | `TIMESTAMPTZ` | no | - | 수신 시각 |

Index:

- unique: `agent_heartbeats.public_id`
- unique: `(device_id, idempotency_key)`
- index: `(device_id, received_at)`
- index: `agent_heartbeats.received_at`
...
| `id` | `BIGINT` | no | - | 내부 PK |
| `public_id` | `UUID` | no | - | 외부 ID |
| `channel` | `VARCHAR(30)` | no | - | V1: `STABLE` |
| `policy_version` | `VARCHAR(80)` | no | - | 정책 버전 |
| `minimum_supported_version` | `VARCHAR(50)` | no | - | 최소 지원 Agent 버전 |
| `latest_version` | `VARCHAR(50)` | no | - | 최신 Agent 버전 |
| `installer_url` | `TEXT` | yes | - | 설치 파일 URL |
| `sha256` | `VARCHAR(128)` | yes | - | 설치 파일 checksum |
| `manifest_signature` | `TEXT` | yes | - | manifest 서명 |
| `kill_switch` | `BOOLEAN` | no | - | 전체 중지 스위치 |
| `created_at` | `TIMESTAMPTZ` | no | - | 생성 시각 |

Index:

- unique: `agent_update_policies.public_id`
- unique: `(channel, policy_version)`
- index: `agent_update_policies.created_at`
...
| `id` | `BIGINT` | no | - | 내부 PK |
| `public_id` | `UUID` | no | - | 외부 ID |
| `policy_id` | `BIGINT` | no | `agent_update_policies.id` | 기준 정책 |
| `version` | `VARCHAR(50)` | no | - | rollout 대상 버전 |
| `status` | `VARCHAR(30)` | no | - | `ACTIVE`, `PAUSED`, `ROLLED_BACK` |
| `started_at` | `TIMESTAMPTZ` | no | - | 시작 시각 |
| `finished_at` | `TIMESTAMPTZ` | yes | - | 종료 시각 |

Index:

- unique: `agent_update_rollouts.public_id`
- index: `agent_update_rollouts.policy_id`
...
| `id` | `BIGINT` | no | - | 내부 PK |
| `public_id` | `UUID` | no | - | 외부 ID |
| `device_id` | `BIGINT` | no | `agent_devices.id` | 업로드 디바이스 |
| `idempotency_key` | `VARCHAR(160)` | no | - | 멱등성 키 |
| `status` | `VARCHAR(30)` | no | - | `QUEUED`, `UPLOADING`, `UPLOADED`, `FAILED_RETRYABLE`, `FAILED_FINAL`, `CANCELLED`, `EXPIRED` |
| `range_started_at` | `TIMESTAMPTZ` | no | - | 로그 구간 시작 |
| `range_ended_at` | `TIMESTAMPTZ` | no | - | 로그 구간 종료 |
...
- unique: `agent_upload_jobs.public_id`
- unique: `(device_id, idempotency_key)`
- index: `agent_upload_jobs.status`
- index: `agent_upload_jobs.created_at`
...
| `id` | `BIGINT` | no | - | 내부 PK |
| `public_id` | `UUID` | no | - | 외부 ID |
| `upload_job_id` | `BIGINT` | no | `agent_upload_jobs.id` | 기준 업로드 작업 |
| `log_upload_id` | `BIGINT` | yes | `agent_log_uploads.id` | 연결 로그 업로드 |
| `schema_version` | `INTEGER` | no | - | 번들 schema 버전. `> 0` CHECK |
| `storage_path` | `TEXT` | no | - | 저장 위치 |
| `sha256` | `VARCHAR(128)` | no | - | 파일 checksum |
| `size_bytes` | `BIGINT` | no | - | 파일 크기. `>= 0` CHECK |
...
| `id` | `BIGINT` | no | - | 내부 PK |
| `public_id` | `UUID` | no | - | 외부 ID |
| `user_id` | `BIGINT` | no | `users.id` | 요청 사용자 |
| `device_id` | `BIGINT` | yes | `agent_devices.id` | 관련 디바이스 |
| `scope` | `VARCHAR(50)` | no | - | `LOCAL_LOGS`, `SERVER_LOGS`, `ALL` |
| `status` | `VARCHAR(30)` | no | - | `REQUESTED`, `PROCESSING`, `COMPLETED`, `FAILED` |
| `idempotency_key` | `VARCHAR(160)` | no | - | 멱등성 키 |
| `reason` | `VARCHAR(80)` | no | - | 요청 사유 |
| `requested_at` | `TIMESTAMPTZ` | no | - | 요청 시각 |
| `processed_at` | `TIMESTAMPTZ` | yes | - | 처리 시각 |
| `error_message` | `TEXT` | yes | - | 실패 요약 |

Index:

- unique: `agent_delete_requests.public_id`
- unique: `(device_id, idempotency_key)`
- index: `agent_delete_requests.user_id`
...
- unique: `visit_support_reservations.public_id`
- index: `visit_support_reservations.as_ticket_id`
...
|---|---|---:|---|---|
| `id` | `BIGINT` | no | - | 내부 PK |
| `agent_device_id` | `BIGINT` | no | `agent_devices.id` | 요청 디바이스 |
| `idempotency_key` | `VARCHAR(160)` | no | - | 요청 멱등성 키 |
| `request_method` | `VARCHAR(10)` | no | - | HTTP method |
| `request_path` | `VARCHAR(512)` | no | - | 요청 path |
| `request_hash` | `CHAR(64)` | no | - | 요청 본문 hash |
| `response_status` | `INTEGER` | yes | - | 저장된 응답 status |
| `response_content_type` | `VARCHAR(120)` | yes | - | 저장된 응답 content type |
| `response_body` | `TEXT` | yes | - | 저장된 응답 본문 |
| `status` | `VARCHAR(30)` | no | - | `IN_PROGRESS`, `COMPLETED` |
| `created_at` | `TIMESTAMPTZ` | no | - | 생성 시각 |
| `updated_at` | `TIMESTAMPTZ` | no | - | 수정 시각 |
| `completed_at` | `TIMESTAMPTZ` | yes | - | 완료 시각 |
| `expires_at` | `TIMESTAMPTZ` | no | - | 기록 만료 시각 |

Index:

- unique: `(agent_device_id, request_method, request_path, idempotency_key)`
- index: `agent_idempotency_records.expires_at`
...
| `upload_job_id` | `BIGINT` | yes | `agent_upload_jobs.id` | 업로드 작업 (V56) |
...
| `failure_category` | `VARCHAR(60)` | no | - | `RECOMMENDATION_BUILD`, `PART_SELECTION`, `COMPATIBILITY`, `PERFORMANCE`, `USER_ENVIRONMENT`, `AGENT_LOG_ONLY`, `OTHER` |
...
- unique: `as_ticket_labels.public_id`
- unique: `as_ticket_labels.as_ticket_id` — 티켓당 라벨 1개
- index: `(use_for_recommendation_training, created_at)`
- index: `as_ticket_labels.related_part_id`
- index: `as_ticket_labels.related_build_id`
...
| `support_decision` | `VARCHAR(50)` | yes | - | `SELF_SOLVABLE`, `REMOTE_POSSIBLE`, `VISIT_REQUIRED`, `NEEDS_MORE_INFO` (V56) |
...
목적: AS Chat/Build Chat 등 Agent 내부 LLM 호출의 profile, model, latency, token, schema 검증 결과를 저장한다.
...
| `ai_profile` | `VARCHAR(60)` | no | - | `AS_CHAT_FAST`, `AS_CHAT_54_FAST`, `AS_CHAT_54_MINI_FAST`, `AS_CHAT_NANO_FAST`, `AS_CHAT_BALANCED`, `AS_CHAT_HIGH_QUALITY`, `BUILD_CHAT_FAST`, `BUILD_CHAT_54_FAST`, `BUILD_CHAT_54_MINI_FAST` (V59) |
| `provider` | `VARCHAR(40)` | no | - | `openai` |
| `model` | `VARCHAR(120)` | no | - | 실제 호출 모델 |
| `reasoning_effort` | `VARCHAR(30)` | yes | - | reasoning effort |
| `use_case` | `VARCHAR(60)` | no | - | `AS_CHAT`, `REQUIREMENT_PARSE`, `BUILD_RECOMMEND`, `BUILD_EXPLAIN`, `AS_ANALYZE` (DB CHECK 없음, 코드 기록 값) |
...
목적: 추천 노출, 클릭, 저장, 변경 적용, 거절, 관리자 확정 AS negative feedback, 관리자 홈 추천부품 라벨을 학습용 이벤트로 저장한다.
...
| `id` | `BIGINT` | no | - | 내부 PK |
| `public_id` | `UUID` | no | - | 외부 ID |
| `user_id` | `BIGINT` | no | `users.id` | 이벤트 대상 사용자 |
| `created_by_admin_id` | `BIGINT` | yes | `users.id` | AS feedback 또는 홈 추천부품 라벨을 확정한 관리자 |
| `event_type` | `VARCHAR(50)` | no | - | 추천 행동(`IMPRESSION`, `CLICK`, `DETAIL_VIEW`, `ADD_PART_TO_DRAFT` 등), `AS_CONFIRMED_NEGATIVE`, `ADMIN_PROMOTE`, `ADMIN_DEMOTE` |
| `label_score` | `NUMERIC(6,2)` | no | - | 학습 label 값 |
| `source_surface` | `VARCHAR(80)` | no | - | `BUILD_CHAT`, `HOME_RECOMMENDED_PARTS`, `SELF_QUOTE`, `ADMIN_AS_FEEDBACK`, `ADMIN_HOME_PART_FEEDBACK` 등 |
| `recommendation_id` | `VARCHAR(160)` | yes | - | UI/AI 추천 stable id |
| `build_id` | `BIGINT` | yes | `builds.id` | 저장된 build가 있는 경우 |
| `part_id` | `BIGINT` | yes | `parts.id` | 관련 부품 |
| `as_ticket_id` | `BIGINT` | yes | `as_tickets.id` | 관리자 확정 AS feedback |
| `model_version_id` | `BIGINT` | yes | `recommendation_model_versions.id` | 관련 모델 버전 |
| `category` | `VARCHAR(50)` | yes | - | 관련 부품 카테고리 |
| `rank_position` | `INTEGER` | yes | - | 노출 순서 |
| `idempotency_key` | `VARCHAR(180)` | yes | - | 중복 이벤트 방지 키 |
| `event_payload` | `JSONB` | no | - | 비민감 메타데이터 |
| `created_at` | `TIMESTAMPTZ` | no | - | 생성 시각 |

Index:

- unique: `recommendation_events.public_id`
- unique partial: `(user_id, idempotency_key)` where `idempotency_key IS NOT NULL`
- index: `(user_id, created_at)`
- index: `(event_type, created_at)`
- index: `recommendation_events.part_id`
- index: `recommendation_events.build_id`
- index: `recommendation_events.as_ticket_id`
...
- positive label은 `SAVE`, `CHANGE_ADOPTED`, `ADD_BUILD_TO_DRAFT`, `ADD_PART_TO_DRAFT`, `ORDER_INTENT`, weak positive는 `CLICK`, `DETAIL_VIEW`, negative는 `REJECT`, `CHANGE_REVERTED`, AS negative는 `AS_CONFIRMED_NEGATIVE`다. 관리자 홈 라벨은 `ADMIN_PROMOTE=+4`, `ADMIN_DEMOTE=-3`으로 학습에 사용한다.

### recommendation_model_versions

목적: batch 학습 또는 shadow scoring에서 사용한 reranker 모델 버전과 metric을 저장한다.
...
| `id` | `BIGINT` | no | - | 내부 PK |
| `public_id` | `UUID` | no | - | 외부 ID |
| `model_name` | `VARCHAR(120)` | no | - | `xgboost-reranker` |
| `model_version` | `VARCHAR(120)` | no | - | 모델 버전 stable key. UNIQUE |
| `algorithm` | `VARCHAR(60)` | no | - | `XGBOOST` |
| `artifact_path` | `TEXT` | yes | - | batch artifact 경로 |
| `status` | `VARCHAR(30)` | no | - | `CANDIDATE`, `SHADOW`, `ACTIVE`, `RETIRED`, `FAILED` |
| `trained_from` | `TIMESTAMPTZ` | yes | - | 학습 데이터 시작 |
| `trained_to` | `TIMESTAMPTZ` | yes | - | 학습 데이터 종료 |
| `metrics` | `JSONB` | no | - | 학습/검증 metric |
| `feature_schema` | `JSONB` | no | - | feature 목록 |
| `created_by` | `BIGINT` | yes | `users.id` | 학습 실행 관리자 |
| `activated_at` | `TIMESTAMPTZ` | yes | - | active 전환 시각 |
| `created_at` | `TIMESTAMPTZ` | no | - | 생성 시각 |
| `deleted_at` | `TIMESTAMPTZ` | yes | - | soft delete |

Index:

- unique: `recommendation_model_versions.public_id`
- unique: `recommendation_model_versions.model_version`
- index: `recommendation_model_versions.status`
- index: `recommendation_model_versions.created_at`
- index: `recommendation_model_versions.deleted_at`

### recommendation_shadow_scores

목적: 현재 서비스 추천 결과에 대해 외부 XGBoost scorer가 매긴 shadow score를 저장한다.
...
| `id` | `BIGINT` | no | - | 내부 PK |
| `public_id` | `UUID` | no | - | 외부 ID |
| `user_id` | `BIGINT` | yes | `users.id` | 요청 사용자 |
| `model_version_id` | `BIGINT` | yes | `recommendation_model_versions.id` | scorer 모델 버전 |
| `source_surface` | `VARCHAR(80)` | no | - | `BUILD_CHAT`, `HOME_RECOMMENDED_PARTS` |
| `request_hash` | `VARCHAR(80)` | no | - | 요청/후보 fingerprint |
| `candidate_type` | `VARCHAR(30)` | no | - | `BUILD`, `PART` |
| `candidate_id` | `VARCHAR(180)` | no | - | 응답 내 stable 후보 id |
| `part_id` | `BIGINT` | yes | `parts.id` | 후보가 부품인 경우 |
| `score` | `NUMERIC(12,6)` | no | - | shadow score |
| `rank_position` | `INTEGER` | yes | - | 기존 노출 순서 |
| `features` | `JSONB` | no | - | scorer에 전달한 feature |
| `raw_response` | `JSONB` | no | - | scorer 응답 일부 |
| `created_at` | `TIMESTAMPTZ` | no | - | 생성 시각 |

MVP 기준 결정값:

- Build Chat에서는 `RECOMMENDATION_RERANKER_ENABLED=false`가 기본이므로 실제 견적 추천 순서를 바꾸지 않는다.
- 홈 하단 추천부품에서는 scorer가 정상 응답하면 `recommendation_shadow_scores`에 점수를 저장하고 해당 score를 화면 랭킹에 반영한다.
- scorer 장애, 미기동, JSON 오류는 추천 API 실패로 전파하지 않는다.
- Tool `FAIL` 또는 하드 조건 위반으로 제외된 후보는 shadow scoring 대상에도 넣지 않는다.
- 홈 하단 추천부품 랭킹은 `parts.status=ACTIVE` 내부 자산만 후보로 사용하고, scorer 장애 시 deterministic fallback 랭킹으로 사용자 화면을 유지한다.
- `recommendation_events.event_payload`에는 HOME 추천 이벤트의 `scoreSource`, `modelVersion`, `reasonTags`, 관리자 라벨 사유 같은 비민감 운영 메타데이터를 평탄화해 저장한다.

### recommendation_training_datasets

목적: 홈 추천부품 XGBoost 학습에 사용할 이벤트 snapshot 묶음을 관리한다.
...
| `id` | `BIGINT` | no | - | 내부 PK |
| `public_id` | `UUID` | no | - | 외부 ID |
| `name` | `VARCHAR(160)` | no | - | 관리자 지정 dataset 이름 |
...
| `included_count` | `INTEGER` | no | - | 학습 포함 item 수 |
| `excluded_count` | `INTEGER` | no | - | 학습 제외 item 수 |
| `created_by` | `BIGINT` | yes | `users.id` | dataset 생성 관리자 |
| `locked_at` | `TIMESTAMPTZ` | yes | - | 학습 전 잠금 시각 |
| `created_at` | `TIMESTAMPTZ` | no | - | 생성 시각 |
| `updated_at` | `TIMESTAMPTZ` | yes | - | 수정 시각 |
| `deleted_at` | `TIMESTAMPTZ` | yes | - | soft delete |
...
목적: dataset에 편입된 추천 이벤트와 학습 시점 feature/label snapshot을 저장한다.
...
| `id` | `BIGINT` | no | - | 내부 PK |
| `public_id` | `UUID` | no | - | 외부 ID |
| `dataset_id` | `BIGINT` | no | `recommendation_training_datasets.id` | 소속 dataset |
| `event_id` | `BIGINT` | no | `recommendation_events.id` | 원본 이벤트 |
| `included` | `BOOLEAN` | no | - | 학습 투입 여부 |
| `excluded_reason` | `VARCHAR(240)` | yes | - | 제외 사유 |
| `label_score_snapshot` | `NUMERIC(6,2)` | no | - | dataset 생성 시점 label |
| `features_snapshot` | `JSONB` | no | - | dataset 생성 시점 feature |
| `event_snapshot` | `JSONB` | no | - | 화면 표시/감사를 위한 원본 이벤트 요약 |
| `created_at` | `TIMESTAMPTZ` | no | - | 생성 시각 |
| `updated_at` | `TIMESTAMPTZ` | yes | - | 수정 시각 |
...
목적: locked dataset 기준 XGBoost 학습 Job의 큐, 실행 상태, metric, artifact 경로를 관리한다.
...
| `id` | `BIGINT` | no | - | 내부 PK |
| `public_id` | `UUID` | no | - | 외부 ID |
| `dataset_id` | `BIGINT` | no | `recommendation_training_datasets.id` | 학습 dataset |
| `model_version_id` | `BIGINT` | yes | `recommendation_model_versions.id` | 성공 시 생성된 SHADOW 모델 |
| `status` | `VARCHAR(40)` | no | - | `QUEUED`, `RUNNING`, `SUCCEEDED`, `FAILED`, `CANCELLED`, `SKIPPED_LOW_DATASET` |
| `worker_id` | `VARCHAR(120)` | yes | - | 처리 worker |
| `queued_by` | `BIGINT` | yes | `users.id` | Job 생성 관리자 |
| `model_version` | `VARCHAR(120)` | yes | - | 생성 모델 버전 문자열 |
| `artifact_path` | `TEXT` | yes | - | 모델 artifact 경로 |
| `metrics` | `JSONB` | no | - | 학습 결과 또는 skip/failure metric |
| `log_summary` | `TEXT` | yes | - | 관리자 표시용 상태/오류 요약 |
| `created_at` | `TIMESTAMPTZ` | no | - | 생성 시각 |
| `started_at` | `TIMESTAMPTZ` | yes | - | 시작 시각 |
| `finished_at` | `TIMESTAMPTZ` | yes | - | 종료 시각 |
| `updated_at` | `TIMESTAMPTZ` | yes | - | 수정 시각 |
...
- API 서버는 Python 학습을 직접 실행하지 않고 `QUEUED` Job만 생성한다.
- `xgb-reranker` worker가 DB에서 `QUEUED` Job을 polling해 처리한다.
- 50건 미만 dataset은 기본적으로 `SKIPPED_LOW_DATASET`으로 끝나며 `recommendation_model_versions`를 만들지 않는다.
- 학습 성공 시 모델은 `recommendation_model_versions.status='SHADOW'`로 생성된다.
- SHADOW 모델은 관리자 activate API가 scorer reload에 성공하기 전까지 홈 추천부품 active ranking에 반영하지 않는다.
...
- `builds.warnings`
- `build_graph_layouts.positions_json`
- `parts.attributes`
- `price_snapshots.raw_payload`
- `part_external_offers.raw_payload`
- `part_catalog_candidates.raw_payload`
- `part_alias_review_items.raw_payload`
- `game_fps_benchmarks.metadata`
- `game_fps_coverage_targets.metadata`
- `manufacturer_sources.parser_config`
- `manufacturer_posts.raw_payload`
- `pipeline_job_runs.result_summary`
...
- `build_chat_semantic_cache.response_payload`
- `llm_generations.request_metadata`
- `agent_log_summaries.summary_payload`, `feature_payload`, `event_counts`, `risk_flags`, `privacy_summary`
- `as_tickets.cause_candidates`
- `as_tickets.upgrade_candidates`
- `as_chat_messages.structured_payload`
- `recommendation_events.event_payload`
- `recommendation_model_versions.metrics`, `feature_schema`
- `recommendation_shadow_scores.features`, `raw_response`
- `recommendation_training_datasets.filters`
- `recommendation_training_dataset_items.features_snapshot`, `event_snapshot`
- `recommendation_training_jobs.metrics`
...
다나와 백업 갱신 작업은 `price_snapshots.source = DANAWA_BACKUP`과 `part_external_offers.source = DANAWA_BACKUP` row만 저장한다. 이 값은 가격변동 추이와 백업 근거로 사용하고 `parts.price`를 변경하지 않는다.

다나와 월별 가격 추이 갱신 작업은 `price_snapshots.source = DANAWA_PRICE_TREND` row만 저장한다. 이 값은 다나와 상품 상세의 6/12/24개월 최저가 그래프에 실제 표시된 포인트와 현재가 포인트를 가격변동 그래프 seed로 쓰기 위한 데이터다. 월별 포인트는 해당 월 1일 00:00 KST, 현재가 포인트는 현재일 12:00 KST를 `collected_at`으로 사용한다. 같은 상품, 같은 source, 같은 `collected_at`은 partial unique index로 upsert한다.

상품별 가격변동 추이는 `price_snapshots` 누적 데이터가 기준이다. 네이버 쇼핑 검색 API와 다나와 공개 검색 페이지는 현재 검색 결과를 가져오는 용도이고, 다나와 상품 상세의 최저가 추이 그래프만 월별 과거 포인트 seed로 사용한다. 따라서 관리자 갱신 작업 또는 서버 스케줄러가 저장한 snapshot을 `GET /api/parts/{id}/price-history`가 조회한다. 다나와 추이 갱신은 월 1회 기본이며, 추출 실패 상품은 임의 보간하지 않고 누락 리포트에 남긴다.
...
공식 제조사 페이지가 403, timeout, 빈 응답 등으로 scan에 실패하면 해당 row는 `manufacturer_sources.status = 'ERROR'`, `error_summary`에 실패 사유를 저장한다. 단, 차단성 응답(403/429)이 연속 3회 누적되면 `ERROR`가 아니라 `status = 'PAUSED'`로 자동 전환해 전체 scan 대상에서 제외하며, 관리자가 원인 확인 후 수동으로 재개한다. 성공 scan은 `consecutive_failures`를 0으로 리셋한다 (V92). 실패는 다른 source scan을 중단시키지 않으며, 관리자는 source URL 또는 개별 scan으로 재검증한다. 차단/캡차/비공개 API 우회는 하지 않는다.
...
V36__part_benchmark_normalized_scores.sql
V37__part_public_raw_benchmarks.sql
V38__game_fps_benchmark_seed.sql
V39__game_fps_seed_quality_hardening.sql
V40__game_fps_coverage_targets.sql
V41__pc_builds_game_fps_seed_expansion.sql
V42__pc_builds_pubg_gpu_gap_seed.sql
V43__rag_vector_index.sql
V44__rag_hard_constraint_seed.sql
V45__danawa_price_snapshot_index.sql
V46__danawa_price_trend_unique_index.sql
V47__danawa_price_trend_seed.sql
V48__danawa_price_trend_manual_pcode_backfill.sql
V49__quarantine_unverified_danawa_price_trends.sql
V50__mark_verified_danawa_price_trends.sql
V51__danawa_price_trend_manual_audit_round2.sql
V52__manufacturer_release_intake.sql
...
V61__rag_shared_price_policy_purposes.sql
V62__recommendation_learning_pipeline.sql
V63__home_part_recommendation_admin_feedback.sql
V64__recommendation_training_operations.sql
V65__rag_quality_v2_failure_seed.sql
V66__build_graph_layouts.sql
V67__build_chat_semantic_cache.sql
V68__agent_log_summary_as_feedback.sql
