...
`GET /api/public/home`은 비로그인 홈 렌더링 전용 읽기 API다. 카테고리별 ACTIVE 부품은 저장된 내부 자산만 사용하고, 추천 부품은 사용자 ID·추천 이벤트·XGBoost 개인화 없이 deterministic fallback 순위만 반환한다. 견적 적용, AI 상담, 부품 상세, 추천 이벤트 기록을 포함한 후속 기능은 기존 로그인 필요 API를 사용한다.
