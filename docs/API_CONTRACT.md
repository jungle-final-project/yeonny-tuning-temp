...
| `POST` | `/api/ai/build-chat/session/reset` | USER | 3번 | - | `{ "status": "RESET" }` | `ai_chat_sessions.context` |
...
- QuoteAgent 모드에서는 로그인 사용자 ID로 현재 `ai_chat_sessions` 행을 조회하고 `context`를 누적한다. 프론트는 `agentSessionId`나 `clarificationContext`를 저장·전송하지 않는다. 사용자가 `새 대화 시작하기`를 누르면 `POST /api/ai/build-chat/session/reset`이 현재 사용자의 `context`만 `{}`로 초기화하며 저장된 견적은 유지한다.
...
- **모호 요청 되묻기(clarification)**: QuoteAgent 모드에서 예산·용도·구체 모델이 모두 없는 견적 요청은 견적을 생성하지 않고 `GENERAL` 응답의 `quickReplies`와 `clarification.missingSlots`로 필요한 조건을 묻는다. 프론트는 사용자의 새 `message`만 전송하며 이전 질문이나 세션 ID를 에코하지 않는다. 서버는 로그인 사용자의 `ai_chat_sessions.context`에 확인된 `budget`, `usageTags`를 누적해 남은 슬롯만 다시 묻고, 조건이 충족되면 추천으로 진행한다. 첫 되묻기와 용도-only 후속의 예산 칩은 ACTIVE 내부 자산의 최소 구성가를 기준으로 동적 생성한다.
