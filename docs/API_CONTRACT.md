...
| `POST` | `/api/admin/recommendation-feedback/as-tickets/{id}` | ADMIN | 3번/4번 협업 | `{ "failureCategory": "PART_SELECTION", "severity": "HIGH", "relatedPartId": "0e9f3b8b-8c83-4d9a-9f7d-1f2b4dfb8a11", "category": "GPU", "useForRecommendationTraining": true, "note": "추천 GPU 발열 문제로 확정" }` | `{ "label": { "id": "7dfb98c8-7f35-4fd3-95e0-dfd58cbda77a", "failureCategory": "PART_SELECTION", "severity": "HIGH", "useForRecommendationTraining": true }, "event": { "id": "9b2f2f18-3b35-45da-8f65-79e4dc101a10", "eventType": "AS_CONFIRMED_NEGATIVE", "sourceSurface": "ADMIN_AS_FEEDBACK" }, "trainingEventCreated": true }` | `as_ticket_labels`, `agent_log_summaries`, `recommendation_events`, `as_tickets` 읽기 |
...
- 명시 예산 Build Chat 추천은 예산 모드를 구분한다. `800만원으로`, `800만원짜리`, `800만원 정도`는 target budget으로 보고 총액을 예산의 87.5%~112.5% 범위에 맞춘다. `이하`, `안으로`, `넘지 않게`는 max budget으로 예산 이하를 우선한다. `이상`, `최소`, `부터`는 min budget으로 기존 하한 예산 동작을 유지한다. 명시 부품 하드 조건이 있는 경우만 예산 초과를 허용하며 `HARD_CONSTRAINT_OVER_BUDGET` warning을 포함한다.
...
- 홈 하단 `인기 부품 랭킹`은 `GET /api/recommendations/home-parts`가 제공하는 내부 자산 추천부품 4개를 사용한다. 이 API는 견적 생성기가 아니라 쇼핑 발견용 부품 랭킹이다. trained model scorer가 정상 응답하면 XGBoost score를 우선 정렬에 반영하고, 모델 파일 없음, baseline-shadow, scorer 실패, 응답 오류인 경우에는 benchmark, 이미지/offer, 가격 갱신 신선도, toolReady, FPS coverage 기반 deterministic fallback을 반환한다.
...
- 50건 미만 dataset은 기본적으로 `SKIPPED_LOW_DATASET`으로 끝나며 모델 버전을 만들지 않는다. 학습 성공 모델은 `SHADOW`로만 저장되고, `POST /api/admin/recommendation-models/{id}/activate`가 scorer reload에 성공해야 `ACTIVE`로 전환된다. 활성 모델 파일은 `/models/home-parts-active.json`로 유지한다.
- 추천 학습 이벤트는 `POST /api/recommendation-events`로 수집한다. 홈 추천부품은 노출 `IMPRESSION`, 클릭 `CLICK`, 상세 진입 `DETAIL_VIEW`, 견적 담기 `ADD_PART_TO_DRAFT`를 기록한다. Public 이벤트 API는 관리자 전용 이벤트인 `AS_CONFIRMED_NEGATIVE`, `ADMIN_PROMOTE`, `ADMIN_DEMOTE`를 거부한다. 관리자는 `/api/admin/recommendation-feedback/home-parts`로 `ADMIN_PROMOTE` 또는 `ADMIN_DEMOTE` 라벨을 추가할 수 있다.
- AS 접수는 자동 negative label이 아니다. 관리자가 `/api/admin/recommendation-feedback/as-tickets/{id}`로 `as_ticket_labels`를 확정하고, `useForRecommendationTraining=true`이며 관련 `part/build/recommendationId`가 있는 경우에만 `AS_CONFIRMED_NEGATIVE`, `sourceSurface=ADMIN_AS_FEEDBACK` 이벤트가 생성된다. 이 API는 `as_tickets.status`, `cause_candidates`, `upgrade_candidates`를 수정하지 않는다.
...
- fast route는 전체 자연어 이해가 아니라 명확한 이동 표현만 처리하는 shortcut이다. 프론트 즉시 처리는 정적 화면 이동과 명확한 category-only 이동으로 제한하고, 상품 상세/필터/시뮬레이션/장바구니 변경 판단은 서버 `BuildChatIntentRouter` decision을 따른다. `GPU 추천해줘`, `GPU 더 싼 걸로`, `이 견적 담아줘` 같은 추천/교체/삭제/담기 명령은 fast route로 처리하지 않고 Build Chat으로 보낸다.
...
- `OPEN_ROUTE` payload는 `{ route, source, reason? }`이며 route는 사용자 화면 allowlist(`/self-quote`, `/self-quote?category=...`, `/self-quote?category=...&q=...`, `/my/quotes`, `/requirements/new`, `/support/new`, `/support/ai-chat`, `/checkout`, 서버가 구체 id를 반환한 `/parts/{partId}`)만 허용한다. 관리자 경로 자동 이동은 금지한다.
- 구체 상품 상세 이동은 단일 고확신 `ACTIVE` 부품일 때만 `/parts/{partId}`로 이동한다. `5090 보여줘`처럼 복수 후보 가능성이 있으면 `/self-quote?category=GPU&q=5090` 같은 후보 필터 화면으로 이동한다.
...
- `바꾸면/교체하면/넣으면/달면/차이/FPS/성능` 계열은 장바구니 mutation보다 우선해 읽기 전용 `simulation`으로 분리한다. target 부품이 명확하지 않으면 임의 후보를 고르지 않고 `SIMULATION_TARGET_NOT_FOUND` warning 또는 follow-up으로 내려가며, simulation 응답은 `actions=[]`를 보장한다.
- `currentQuoteDraft`가 전달된 `/self-quote` 챗봇 요청은 `actions`로 변경안을 반환한다. AI API는 `quote_drafts`, `quote_draft_items`를 직접 쓰지 않는다. 프론트 자동 실행은 `OPEN_ROUTE` 또는 draft mutation action의 `payload.intentConfidence=HIGH` 및 `payload.sideEffectRisk=LOW`인 경우로 제한한다. 그 외 변경 후보는 카드로 남기고 즉시 저장하지 않는다.
