...
- 공개 `/api/ai/build-chat` 응답은 `actions` 필드를 반환하지 않는다. 내부 `AiChatEngine`의 action(`OPEN_SELF_QUOTE`, `ADD_BUILD_TO_DRAFT`, `CREATE_PRICE_ALERT` 등)은 서버 내부용이며 공개 응답 shape에 노출하지 않는다. 견적 변경은 '변경 미리보기' 카드를 먼저 만들고, 카드 버튼 또는 직후 고확신 자연어 확인을 받은 프론트가 `PUT /api/quote-drafts/current/apply-ai-build`를 호출해 수행한다.
