...
- **점수 근거에서 이어지는 케이스 개선 추천**: 현재 견적에 케이스 장착 여유 또는 명시된 통풍 구성으로 인한 score cap이 있을 때 `현재 부품이 여유 있게 들어가는 케이스 추천해줘`, `통풍형 케이스 추천해줘`를 요청하면 일반 가격 TOP3를 그대로 반환하지 않는다. 서버가 각 ACTIVE 케이스로 현재 견적을 다시 평가해 Tool `FAIL`이 없고, 케이스 cap과 최종 종합 점수가 모두 실제로 상승하는 후보만 최대 3개 반환한다. 현재 케이스와 장착 수치가 같은 파생 상품은 제외하며, 검증된 개선 후보가 없으면 임의 후보를 만들지 않는다. 현재 케이스가 이미 통풍형이고 실제 제한 원인이 PSU/GPU/쿨러 치수라면 통풍 부족이라고 설명하지 않고 해당 부품의 실제 길이·케이스 허용 길이·여유 mm를 안내한다.
...
| `GET` | `/api/users/me/agent-diagnosis-requests/latest` | USER | 4번 | - | `{ "diagnosis": { "diagnosisId": "...", "currentProgress": 100, "completed": true, "result": {...}, "asTicket": { "id": "...", "status": "OPEN", "reviewStatus": "REQUIRED", "supportDecision": null, "createdAt": "..." } } }`; 이력이 없으면 `{ "diagnosis": null }` | `pc_agent_diagnosis_requests`, `pc_agent_diagnosis_events`, `pc_agent_diagnosis_results`, `as_tickets` |
...
| `GET` | `/api/admin/as-tickets/{id}` | ADMIN | 4번 | - | `{ "id": "4aef8ef7-1dc7-45d1-bfc2-bb0cfdaf7f8a", "diagnosisId": "9a0e3c21-6648-41e7-a88e-17be1761b806", "status": "OPEN", "diagnosisMode": "DEMO", "diagnosisTitle": "그래픽 장치 오류 상태가 확인되었습니다", "diagnosisSummary": "...", "diagnosisEvidence": [...], "diagnosisResult": { "recommendedActions": [...] }, "diagnosisEvents": [{ "eventId": "...", "progressPercent": 100, "message": "Code 43 진단 완료" }], "supportRouting": { "recommendedService": "REMOTE_SUPPORT" }, "assignedAdminId": null }` | `as_tickets`, `users`, `pc_agent_diagnosis_events` |
...
- 사용자 진단 조회는 인증 토큰에서 확정한 `user_id`를 모든 조회 조건에 포함한다. `/latest`는 해당 사용자의 `requested_at` 기준 최신 진단 한 건만 반환하고, 연결 AS 티켓도 같은 `user_id`와 `diagnosis_id`가 모두 일치할 때만 포함한다. `supportDecision=null`은 관리자 결정 전 상태이며 방문 지원 확정을 의미하지 않는다.
