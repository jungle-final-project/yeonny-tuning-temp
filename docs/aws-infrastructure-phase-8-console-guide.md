...
- [x] Phase 7 Green Web·API·WebSocket 검증이 유지됨
- [x] Blue EC2·Public IP·Compose 무변경
- [x] 기존 Blue CloudFront Origin·behavior 무변경
- [x] API ECR repository 생성
- [x] XGB ECR repository 생성
- [x] 두 ECR repository tag immutability 활성
- [x] 두 ECR repository scan on push 활성
- [ ] ECR lifecycle에서 최근 30개 image 보존
- [x] GitHub OIDC provider 생성 또는 기존 provider 확인
- [x] OIDC audience가 `sts.amazonaws.com`
- [x] OIDC Role trust가 `jungle-final-project/prototype`의 `main`만 허용
- [x] GitHub Role이 두 ECR repository에만 push 가능
- [x] GitHub Role이 Green S3 bucket에만 배포 가능
- [x] GitHub Role이 Green CloudFront만 invalidation 가능
- [x] GitHub Role이 Green EC2에만 SSM SendCommand 가능
- [x] GitHub Role에 Secret 읽기·SSH·관리자 권한 없음
- [x] GitHub repository variable 등록
- [x] AWS 장기 Access Key를 GitHub Secret에 추가하지 않음
- [x] `compose.api.ecr.prod.yaml` 추가
- [x] ECR Compose service가 `nginx`, `api`, `xgb-reranker`만 존재
- [x] ECR Compose의 API·XGB에 `build:` 없음
- [x] API·XGB image가 40자리 Git SHA tag 사용
- [x] `latest` tag 없음
- [x] API 8080 host 공개 없음
- [x] XGB 8091 host 공개 없음
- [x] Nginx만 host port 80 사용
- [x] Green 배포 validator test PASS
- [x] 기존 infrastructure validator PASS
- [x] ECR Compose config PASS
- [x] Web workflow 독립 실행 PASS
- [x] `index.html` no-cache 유지
- [x] hash asset 1년 immutable 유지
- [ ] API workflow OIDC·ECR·SSM 배포 PASS
- [x] XGB workflow OIDC·ECR·SSM 배포 PASS
- [x] Green EC2의 API·XGB가 ECR image로 실행됨
- [x] `/opt/buildgraph/green-images.env`가 `600 ubuntu:ubuntu`
- [x] `.env.prod`가 `600 ubuntu:ubuntu`
- [x] Green `/healthz`, `/api/health` PASS
- [x] Green CloudFront `/api/health` PASS
- [ ] API rollback drill PASS
- [x] Web 배포가 API·XGB를 재시작하지 않음
- [x] API 배포가 XGB를 재시작하지 않음
- [x] XGB 배포가 API를 재시작하지 않음
- [x] SSM command target이 Green instance 하나뿐임
- [x] SSM output에 Secret value 없음
- [ ] `GREEN_CD_ENABLED=true` 전환 후 path 기반 자동 배포 확인
- [x] Blue용 `deploy-compose.yml`과 SSH Secret 유지
