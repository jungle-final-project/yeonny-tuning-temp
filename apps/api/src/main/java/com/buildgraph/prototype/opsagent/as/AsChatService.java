package com.buildgraph.prototype.opsagent.as;
...
public class AsChatService {
    ...
    public Map<String, Object> send(
        ...
        try {
            ...
            stageTimings.mark("llmRunningMs");
            GeneratedAsChatJson generated = generateAsChatJson(
                    agentInternalId,
                    aiProfile,
                    ticket,
                    chatSession,
            ...
            Map<String, Object> llmJson = generated.payload();
            ...
            stageTimings.mark("doneMs");
            llmGenerationService.updateStageTimings(generated.llmGenerationId(), stageTimings.snapshot());
    ...
    private GeneratedAsChatJson generateAsChatJson(
        ...
        try {
            ...
            stageTimings.put("llmOnlyLatencyMs", llmResult.latencyMs());
            Map<String, Object> llmJson = strictJson(llmResult.text());
            String llmGenerationId = llmGenerationService.recordSuccess(agentInternalId, aiProfile, AS_CHAT_SCHEMA_NAME, llmResult, stageTimings.snapshot());
            return new GeneratedAsChatJson(llmJson, llmGenerationId);
        } catch (ResponseStatusException error) {
            boolean schemaValid = error.getStatusCode().value() != HttpStatus.BAD_GATEWAY.value();
            long latencyMs = llmResult == null ? elapsedMs(startedAt) : llmResult.latencyMs();
            stageTimings.put("llmOnlyLatencyMs", latencyMs);
    ...
    private static Map<String, Object> responseLimits(AiProfileDefinition aiProfile) {
        return switch (aiProfile.profile()) {
            case AS_CHAT_FAST, AS_CHAT_54_FAST, AS_CHAT_54_MINI_FAST, AS_CHAT_NANO_FAST -> MockData.map(
            ...
            case BUILD_CHAT_FAST, BUILD_CHAT_54_FAST, BUILD_CHAT_54_MINI_FAST -> MockData.map(
                    "assistantMessage", "Korean, around 220 characters",
                    "causeCandidatesMax", 1,
                    "nextActionsMax", 2
            );
    ...
    private record GeneratedAsChatJson(Map<String, Object> payload, String llmGenerationId) {
    }
    ...
    private static final class StageTimings {
        ...
        private StageTimings(long startedAt) {
            this.startedAt = startedAt;
        }

        static StageTimings start() {
            return new StageTimings(System.nanoTime());
        }

        void mark(String key) {
            put(key, elapsedMs(startedAt));
        }
