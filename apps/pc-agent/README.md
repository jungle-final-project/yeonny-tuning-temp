...
진단 작업이 유효하게 완료되면 `DiagnosisRuleEngine`이 저장된 측정 evidence만 사용해 결정적인 `DiagnosisResult`를 생성합니다. 센서 미지원·권한 부족·조회 실패는 이상 근거와 분리하고, GPU 냉각 이상은 고온·고부하·지원되는 팬 센서의 정지·열 제한 또는 클럭 저하 같은 복수 근거가 있을 때만 물리 점검으로 분류합니다. 결과는 로컬에 먼저 저장한 뒤 같은 `resultId`로 서버에 재전송할 수 있으며, 결과 화면의 AS 버튼은 실제 근거가 있는 `PHYSICAL_INSPECTION` 결과에서만 표시됩니다.
...
- 장치별 CPU/GPU 온도 정책과 SMART 공급자 확장
