...
export function ChangePartPage() {
  ...
  const { data: partsPage, isLoading: partsLoading } = useQuery({
  ...
  return (
    <Screen>
      <div className="grid grid-cols-[330px_1fr_360px] gap-5">
        ...
        <Panel title="부품 변경 비교 / 검증" subtitle={`Build ${build?.name ?? buildId}`}>
          ...
          {compareMutation.isPending ? <StateMessage type="info" title="비교 실행 중" body="선택한 부품으로 구성과 Tool 검증 결과를 다시 계산하고 있습니다." /> : null}
          {compareMutation.isError ? <StateMessage type="warn" title="비교 실패" body="선택한 부품의 변경 비교 API를 불러오지 못했습니다." /> : null}
          {comparison ? (
            <div className="space-y-5">
              <DataTable columns={['label', 'before', 'after', 'diff', 'status']} rows={comparison.diffRows.map((row) => ({
                label: row.label,
                before: row.before,
                after: row.after,
                diff: row.diff,
              ...
              <DataTable columns={['category', 'name', 'price']} rows={comparison.afterBuild.items.map((item) => ({
        ...
        <Panel title="적용 결과">
          {comparison ? (
            ...
            <StateMessage type="info" title="후보 선택 필요" body="비교할 부품을 선택하면 적용 가능 여부와 Tool 결과가 표시됩니다." />
          )}
        </Panel>
      </div>
    </Screen>
