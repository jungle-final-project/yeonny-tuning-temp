...
export function listAssemblyRequests(page = 0, size = 20) {
  return api<AssemblyRequestPage>(`/api/assembly-requests?page=${page}&size=${size}`).catch((error) => {
...
export function getTechnicianProfile() {
  return api<Technician | undefined>('/api/technician/profile')
...
export function listTechnicianRequests(scope: 'OPEN' | 'SELECTED', page = 0, size = 20) {
  return api<TechnicianRequestPage>(`/api/technician/assembly-requests?scope=${scope}&page=${page}&size=${size}`).catch((error) => {
    if (error instanceof ApiError && error.status === 404) {
      return { items: [], page, size, total: 0 };
    }
    throw error;
