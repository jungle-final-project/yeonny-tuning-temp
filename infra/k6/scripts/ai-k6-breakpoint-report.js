...
const inputPath = process.argv[2] || path.join('infra', 'k6', 'results', 'ai-chat-breakpoint.ndjson');
const outputPath = process.argv[3] || path.join('infra', 'k6', 'reports', 'ai-chat-breakpoint-report.html');
const bucketMs = 20_000;
const bucketSeconds = bucketMs / 1000;
const reportDurationSeconds = Number(process.argv[4] || process.env.K6_REPORT_DURATION_SECONDS || 120);
if (!Number.isFinite(reportDurationSeconds)
    || reportDurationSeconds <= 0
    || reportDurationSeconds % bucketSeconds !== 0) {
  console.error(`report duration must be a positive multiple of ${bucketSeconds} seconds.`);
  process.exit(1);
}
const reportDurationMs = reportDurationSeconds * 1000;
...
<head>
  ...
  <title>AI Chat Breakpoint Report</title>
...
<body>
  <h1>AI Chat Breakpoint Report</h1>
